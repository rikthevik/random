...
defmodule Point do
  # Tuple gives you less typing in creation {x ,y} rather than %Point{x: x, y: y}
  # Struct gives you p.x and p.y, but is there a creation shorthand?
  def new(x, y) do
    {x, y}
  end

  def mdist({px, py}) do
    Kernel.abs(px) + Kernel.abs(py)
  end
...
defmodule Problem do
  ...
  def line_to_moves(line) do
    line 
    |> String.split(",")
    |> Enum.map(&parse_move/1)    
  end

  def parse_move(chunk) do
    dir = chunk |> String.at(0) |> String.downcase() |> String.to_atom()
    len = chunk |> String.slice(1..100) |> String.to_integer()
    {dir, len}
  end

  def move_to_points({dir, len}) do
    # produce all of the new points (not including origin) for this move
    case dir do
      :u -> for y <- 1..len, do: {0, +y}
      :d -> for y <- 1..len, do: {0, -y}
      :r -> for x <- 1..len, do: {+x, 0}
      :l -> for x <- 1..len, do: {-x, 0}
    end
  end
  
  def moves_to_wire(moves) do
    moves_to_wire({0, 0}, moves)
  end
  def moves_to_wire(p, [move|remaining_moves]) do
    # move from p to the end position, do not include the origin
    points = for move_point <- move_to_points(move), do: p |> Point.add(move_point)
    last_point = List.last(points)
    points ++ moves_to_wire(last_point, remaining_moves)
  end
  def moves_to_wire(p, []) do [] end
  
  def draw_grid(points) do
    # this is mostly for my visual debugging
    {min_x, max_x} = (for {x, y} <- points, do: x) |> Enum.min_max
    {min_y, max_y} = (for {x, y} <- points, do: y) |> Enum.min_max
    for y <- max_y+1..min_y-1 do   # start from the top because we're drawing down
      for x <- min_x-1..max_x+1 do
        cond do
          {x, y} == {0, 0} -> "@"
          points |> MapSet.member?({x, y}) -> "x"
          true -> "."
        end |> IO.write
      end
      "" |> IO.puts
  ...
  def run(inputs) do
    # I was going to do line intersections here, but if we're doing
    # integers, let's just put everything into a set.  Might have overdone
    # it with the Point and Line stuff...
    
    "INPUT" |> IO.puts
    inputs |> IO.inspect
    wire1 = inputs |> Enum.at(0) |> Problem.moves_to_wire |> MapSet.new
    # wire1 |> IO.inspect
    wire2 = inputs |> Enum.at(1) |> Problem.moves_to_wire |> MapSet.new
    # wire2 |> IO.inspect

    all_points = wire1 |> MapSet.union(wire2)
    # all_points |> draw_grid
  
    overlaps = wire1 |> MapSet.intersection(wire2) |> IO.inspect
    overlaps 
    |> Enum.map(&Point.mdist/1)
    |> Enum.min
  end
...
defmodule Tests do 
  ...
  test "tests" do
    assert "R123" |> Problem.parse_move == {:r, 123}
    assert "R123,U23" |> Problem.line_to_moves == [{:r, 123}, {:u, 23}]
    assert [{:u, 2}, {:l, 1}] |> Problem.moves_to_wire == [{0, 1}, {0, 2}, {-1, 2}]
    assert [{:u, 2}, {:l, 1}, {:r, 3}] |> Problem.moves_to_wire == [{0, 1}, {0, 2}, {-1, 2}, {0, 2}, {1, 2}, {2, 2}]
    # == %MapSet{{0, 0}, {0, 1}, {0, 2}, {-1, 2}}
  end
