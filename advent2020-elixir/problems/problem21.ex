...
defmodule Problem do
  ...
  def go(recipes) do
    ...
    case least_count do
      9999 ->
        ...
        recipes
      least_count ->
        least_allergens = recipes
        |> Enum.filter(fn a -> req_count(a) == least_count end)
        |> Enum.map(fn {_, reqs} -> reqs end)
        |> Enum.concat
        |> MapSet.new
        |> Enum.sort

        remove_allergen(recipes, least_allergens)
        |> go
    end
  ...
  def remove_allergen(recipes, [least_allergen|least_allergens]) do

    "remove_allergen #{inspect least_allergen} recipes=" |> IO.write
    recipes |> IO.inspect
