...
def check_sue(sue):
    for k, v in sue.iteritems():
        if k in target_sue:
            if k in ("cats", "trees") and v > target_sue[k]:
                pass
            elif k in ("pomeranians", "goldfish") and v < target_sue[k]:
                pass
            elif target_sue[k] != v:
                return False
    return True
...
def main(inputstr):
    good_sues = []
    for idx, l in enumerate(inputstr.splitlines()):
        sue = { k:int(v) for k, v in [ b.split(": ") for b in l[l.find(": ")+2:].split(", ") ] }
        print "Sue %d: %s" % (idx+1, check_sue(sue))
        if check_sue(sue):
            good_sues.append((idx+1, sue))
    print good_sues
        # break

main("""Sue 1: cars: 9, akitas: 3, goldfish: 0
