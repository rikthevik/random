...
defmodule Part1 do
  ...
  def fold_over(pmap, {"x", foldx}) do
    left = Enum.filter(pmap, fn {x, _} -> x < foldx end)
    right = Enum.filter(pmap, fn {x, _} -> x > foldx end)

    to_add = for {x, y} <- right do
      {2 * foldx - x, y}
    end
