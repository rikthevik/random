...
defmodule Part1 do
  def run(rows) do
    rows
    |> Enum.map(fn [_inputs, outputs] -> outputs end)
    |> List.flatten
    |> Enum.filter(&part1_criteria/1)
    |> Enum.count
  end

  def part1_criteria(s) do
...
defmodule Part2 do
  ...
  def decode(digits) do
    digits
    |> Enum.reverse
    |> Enum.zip(0..100)
    |> Enum.map(fn {digit, pow} ->
...
defmodule Tests do
  ...
  test "example" do
    ...
    assert 26 == input |> prepare |> Part1.run
    assert 61229 == input |> prepare |> Part2.run
  end

  test "go time" do
    input = "dcagfb cbegda cabdf bagedf dcb cd dcbegaf aebcf bgdaf dgcf | dacebgf cbafd dbacf cdgf
