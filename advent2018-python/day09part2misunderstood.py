...
def go(num_players, last_marble):
    ...
    for turn in range(last_marble):
        ...
        if turn == 0:
            ...
            if a_new_marble % 23 == 0:
                ...
                score_record[player_idx].append(marble_score)
            else:
    ...
    print(scores)
    print(max(scores))
    print("indx", scores.index(max(scores)))
    for s in score_record:
        print(sum(s), s)
    hiscores = score_record[scores.index(max(scores))]
    print("HISCORES", hiscores)
    print(sum(hiscores[:-1]) + hiscores[-1]*100)
