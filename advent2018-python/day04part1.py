...
data = collections.defaultdict(lambda: [ 0 for f in range(60) ])
guard = -1
sleepbegin = -1
for l in sorted(inputstr.strip().splitlines()):
    b = regex.match(l).groups()
    year, month, day, hour, minute = map(int, b[:5])
    rest = b[5]
    # print(rest)
    if rest.startswith('#'):
        guard = int(rest[1:])
        ...
        for i in range(sleepbegin, minute):
            data[(guard, year, month, day)][i] = 1

    # print(year, month, day, hour, minute)

guard_times = []
guard_map = {}
for g, l in itertools.groupby(sorted(data.items()), key=lambda a: a[0][0]):
    guard_map[g] = [ times for b, times in l ]
    guard_times.append((g, sum(sum(t) for t in guard_map[g])))

# print(guard_times)
the_guard = list(sorted(guard_times, key=lambda a: a[1], reverse=True))[0][0]

minute_counts = []
for minute, r in enumerate(zip(*guard_map[the_guard])):
    minute_counts.append((sum(r), minute))

print(the_guard * max(minute_counts)[1])
